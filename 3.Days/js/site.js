$(document).ready(function () {
        ...
        res.articles.forEach( item => {
            ...
            `<div class="col-sm-4">
                <div class="card">
                    ...
                    <div class="card-body">
                    <h5 class="card-title  tracking-in-expand ">`+item.title+`</h5>
    ...
    // sources list
    /*
    const sUrl = "https://newsapi.org/v2/sources";
    const sParams = {
        apiKey: "38a9e086f10b445faabb4461c4aa71f8"
    }
    */
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var hmtl = "";
            response.sources.forEach( item => {
                const st = `<option data-tokens="`+item.id+`">`+item.name+`</option>`
                hmtl += st;
            });
            $("#sources").html(hmtl)
    ...
    $.ajax({
        type: "get",
        url: "https://newsapi.org/v2/everything?q=bitcoin&apiKey=38a9e086f10b445faabb4461c4aa71f8",
        dataType: "json",
        success: function (response) {
    ...
    function modalShow () { 
        const statu = sessionStorage.getItem("statu")
        if ( statu == null ) {
    ...
    const stSource = `{
        ...
        localStorage.setItem("data", stSource);
        */

       const dataSource = localStorage.getItem("data")
       if ( dataSource != null ) {
